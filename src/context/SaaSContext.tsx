    ...
    const initialize = async () => {
      if (isSupabaseConfigured) {
        try {
          ...
          if (user) {
            // Dynamic Storage Bucket verification (only for logged-in merchants)
            await supabaseStorage.ensureBucketExists().catch(e => console.warn(e));
            ...
            // Unauthenticated user (e.g. walk-in customer).
            // We must still load the public shops so they can view the customer portal!
            try {
              const dbShops = await supabaseDb.fetchShops();
              if (dbShops.length > 0) {
                setShops(dbShops);
              ...
              console.warn('Failed to load public shops from Supabase, using fallback:', err);
              restoreLocalFallback();
            }
          }
        } catch (error) {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    if (isSupabaseConfigured) {
      try {
        ...
        // Handle automated provisioning for the default demo account in fresh Supabase projects
        if (email.toLowerCase() === 'demo@printflow.cloud' && password === 'demo') {
          try {
            await supabaseAuth.signIn(email, password);
          } catch (signInErr: any) {
            console.log('Demo account not found in active Supabase project. Provisioning on-the-fly...');
            try {
              await register(
                'Demo Owner',
                'Demo Print Shop',
                'demo@printflow.cloud',
                '+1 (555) 019-2831',
                '101 Digital Press Way, Suite A',
                'demo'
              );
              return true;
            } catch (regErr: any) {
              console.error('Failed to auto-register demo shop:', regErr);
              // Fallback to re-throwing original signInErr
              throw signInErr;
            }
          }
        } else {
